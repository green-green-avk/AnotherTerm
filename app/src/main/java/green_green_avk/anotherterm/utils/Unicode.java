...
public final class Unicode {
    ...
    private static boolean biSearch(final int ucs) {
        int min = 0;
        int mid;
        int max = COMBINING.length / 2 - 1;

        if (ucs < COMBINING[0] || ucs > COMBINING[max * 2 + 1]) {
            return false;
        }
        while (max >= min) {
            mid = (min + max) / 2;
            if (ucs > COMBINING[mid * 2 + 1]) {
                min = mid + 1;
            } else if (ucs < COMBINING[mid * 2]) {
                max = mid - 1;
            } else {
                return true;
            }
    ...
    public static int wcwidth(final int codePoint) {
        ...
        return ((codePoint >= 0x1100 &&
                (codePoint <= 0x115f ||                    // Hangul Jamo init. consonants
                        ...
                        (codePoint >= 0x1F000 && codePoint <= 0x1FFFD) || // Emojis & beyond
